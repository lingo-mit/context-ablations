...
class DataCollatorForLanguageModeling:
    ...
    def _tensorize_batch(self, examples: List[torch.Tensor]) -> torch.Tensor:
        ...
        elif all(x.size(0) == length_of_first for x in examples[:-1]):
            return torch.stack(examples[:-1], dim=0)
        else:
            print("length of first: ", length_of_first)
            for x in examples:
                if x.size(0) != length_of_first:
                    print("incorrect size: ", x.size(0))
                    print(x)
